...
test("operator-only live integrations remain present behind explicit configuration", async () => {
  ...
  for (const source of [ask, goals, oracle, briefing]) {
    assert.match(source, /process\.env\.OPERATOR_LIVE_MODE !== "enabled"/);
  }
  assert.match(ask, /streamText\(/);
  assert.match(goals, /conn\.getCollection\("default"\)/);
